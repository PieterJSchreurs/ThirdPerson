...
void GameObject::setEulerAngles(glm::vec3 pRotation)
{
	...
	float radX = pRotation.x * (3.14159f / 180.0f);
	float radY = pRotation.y * (3.14159f / 180.0f);
	float radZ = pRotation.z * (3.14159f / 180.0f);
	...
	glm::mat4 setPosition = glm::mat4(
		...
		glm::vec4(getWorldTransform()[3].x, getWorldTransform()[3].y, getWorldTransform()[3].z, 1)
	);
