...
void Ship::ShootInDir(glm::vec2 pDir, GridGenerator* pGridGen) {
	if (!_shotThisTurn && _actionsRemaining > 0)
	{
		...
		if (!_isAI) {
			glm::vec3 currentPlace = _indicator[_actionsRemaining]->getLocalPosition();
			_indicator[_actionsRemaining]->setLocalPosition(glm::vec3(currentPlace.x, -10, currentPlace.z)); // sets the things below the board, not a nice solution but it's okay.
		}
		if (_isBig)
		{
			if (_isAI)
